...
export function ChartTooltip({ active, payload, label }: ChartTooltipProps) {
  if (active && payload && payload.length) {
    ...
    const formattedDate = label ? (() => {
      try {
        const timestamp = typeof label === 'number' ? label : Number.parseInt(label.toString(), 10);
        return dayjs(timestamp).isValid() ? dayjs(timestamp).format('YYYY-MM-DD HH:mm:ss') : "错误的时间戳";
      } catch (e) {
