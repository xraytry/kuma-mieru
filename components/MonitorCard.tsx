...
export function MonitorCard({ monitor, heartbeats, uptime24h }: MonitorCardProps) {
  ...
  const StatusIcon =
    status === 1 ? CheckCircle2
    : status === 2 ? MinusCircle
    : AlertCircle;
  const statusText =
    status === 1 ? "在线"
    : status === 2 ? "维护中"
  ...
  const uptimeData = [
    {
      value: uptime24h * 100,
      fill:
        uptime24h > 0.98 ? "#17c964"
        : uptime24h > 0.9 ? "#f5a524"
        : "#f31260",
    },
  ];
        ...
        <div className='flex flex-col gap-2'>
          <div className='flex items-center gap-2'>
            <Badge
              ...
              <StatusIcon
                size={18}
                className={`text-${chartColor} h-4 w-4`}
              />
            </Badge>
